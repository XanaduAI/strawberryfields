...
=========================
Graphs can be used to model a wide variety of objects: social networks, financial markets,
biological networks, and many others. A common problem of interest is to find subgraphs that
contain a large number of connections between its nodes. These may correspond to communities in
...
# graph. The two graphs are joined by selecting 8 vertices at random from both graphs and adding an
...
# the :func:`~.search` function in the :mod:`~gbs.subgraph` module: to identify collections of dense
# subgraphs for a range of sizes. The output of this function is a dictionary whose keys correspond
# to subgraph sizes within the specified range. The items in the dictionary are the top subgraphs of
