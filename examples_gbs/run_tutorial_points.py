...
`hafnians <https://en.wikipedia.org/wiki/Hafnian>`__. In this tutorial, we use the permanental point
process in which the probability of observing a pattern of points :math:`S` depends on the
permanent of their corresponding kernel matrix :math:`K_S` as :cite:`jahangiri2019point`:
...
Plot_1 = plot.plot_points(R, samples[0], point_size=10)
...
# sample points from the dense regions with a higher probability. This feature of the GBS point
...
# standard deviation of 0.15 and are centered at :math:`[x = 2, y = 2]` and :math:`[x = 4,
# y = 4]`, respectively. We also add 25 randomly generated points to the data set.

clusters = make_blobs(n_samples=100, centers=[[2, 2], [4, 4]], cluster_std=0.15)[0]
