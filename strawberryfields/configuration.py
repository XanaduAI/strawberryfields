...
import toml
from appdirs import userconfig_dir
...
def load_config(filename="config.toml", **kwargs):
    ...
    config = update_config_from_environmental_variables(_config)
...
def update_config_from_config_file(config, config_file):
    ...
    for section, sectionconfig in _config.items():
...
def look_for_config_file(filename="config.toml"):
    ...
    envconfig_dir = os.environ.get("SF_CONF", "")
    userconfig_dir = user_config_dir("strawberryfields", "Xanadu")

    directories = [current_dir, envconfig_dir, user_config_dir]
