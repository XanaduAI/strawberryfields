...
def sample_tmsv(
    ...
    r"""Generate samples for simulating vibrational quantum dynamics with an input two-mode squeezed state.

    Given N two-mode squeezing parameters and N-dim normal-to-local transformation matrix, Ul,
    a GBS device with 2N modes is simulated. The evolution operator acts on only the first N
    modes in the device. A sample is generated by measuring the number of photons in each of
    the :math:`2N` modes.
    ...
    **Example usage:**
    ...
    [[0, 0, 0, 0], [0, 1, 0, 1], [0, 2, 0, 2], [0, 0, 0, 0], [1, 1, 1, 1]]

    Args:
        r (list): list of two-mode squeezing parameters
        t (float): time in femtoseconds
        Ul (array): normal-to-local transformation matrix
        w (array): normal mode frequencies :math:`\omega` in units of :math:`\mbox{cm}^{-1}`
        n_samples (int): number of samples to be generated
