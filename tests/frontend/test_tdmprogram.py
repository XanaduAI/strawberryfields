...
class TestSingleLoopNullifier():
    """Groups where a nullifier associated with a state generated by a oneloop setup is checked."""
...
class TestTDMcompiler:
    def test_tdm_wrong_layout(self):
        ...
        with pytest.raises(sf.program_utils.CircuitError, match="The gates or the order of gates used in the Program"):
            prog.compile(device=device, compiler="tdm")
    ...
    def test_tdm_wrong_parameter_second_argument(self):
        ...
        prog = tdmprogram.TDMProgram(N=2)
        with prog.context(alpha, phi, theta, copies=copies) as (p, q):
            ops.Sgate(sq_r, 0.4) | q[1] # Note that the Sgate has a second parameter that is non-zero
            ops.BSgate(p[0]) | (q[1], q[0])
            ops.Rgate(p[1]) | q[1]
            ops.MeasureHomodyne(p[2]) | q[0]
        eng = sf.Engine("gaussian")
    ...
    def test_tdm_inconsistent_concurrent_modes(self):
        ...
        prog = singleloop_program(sq_r, alpha, phi, theta, copies)
        with pytest.raises(
            sf.program_utils.CircuitError, match="temporal modes, but the device"
        ):
            prog.compile(device=device, compiler="tdm")

    def test_tdm_inconsistent_concurrent_modes(self):
        device_spec1 = copy.deepcopy(device_spec)
        device_spec1["modes"]["concurrent"] = 100 #Note that singleloop_program has only two concurrent modes
        ...
        prog = singleloop_program(sq_r, alpha, phi, theta, copies)
        with pytest.raises(
            sf.program_utils.CircuitError, match="concurrent modes, but the device"
        ):
            prog.compile(device=device1, compiler="tdm")

    def test_tdm_inconsistent_spatial_modes(self):
        device_spec1 = copy.deepcopy(device_spec)
        device_spec1["modes"]["spatial"] = 100 #Note that singleloo_program has only one spatial mode
        device1 = DeviceSpec("x", device_spec1, connection=None)
        c = 1 # Note that we are requesting more temporal modes (2*c = 200) than what is allowed.
        copies = 1
        sq_r = 0.5643
        ...
        prog = singleloop_program(sq_r, alpha, phi, theta, copies)
        with pytest.raises(
            sf.program_utils.CircuitError, match="spatial modes, but the device"
        ):
            prog.compile(device=device1, compiler="tdm")