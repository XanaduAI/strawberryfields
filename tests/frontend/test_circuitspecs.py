...
class TestAbstractCircuitSpec:
    ...
    @pytest.mark.parametrize("compiler", ["gbs", "dummy"])
    def test_allow_imperfections_warning(self, compiler):
        """Test the gbs and circuitspecs compile methods warns when allowing imperfections"""

        class Dummy(sf.circuitspecs.gbs.GBSSpecs):
            """Dummy circuit used to instantiate
            the abstract base class"""
            compile = CircuitSpecs.compile

        compiler_name = compiler
        if compiler == "dummy":
            compiler = Dummy()
            compiler_name = Dummy().short_name

        prog = sf.Program(2)
        with prog.context as q:
            sf.ops.MeasureFock() | q

        with pytest.warns(UserWarning, match="The {} compile method does not currently support imperfections".format(compiler_name)):
            prog.compile(compiler, allow_imperfections=True)
