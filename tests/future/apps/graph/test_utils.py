...
r"""
Unit tests for glassonion.graphs.utils
"""
...
class TestToNetworkXGraph:
    ...
    def test_valid_adjacency(self, adj, monkeypatch):
        ...
        with monkeypatch.context() as m:
            m.setattr(glassonion.graph.utils, "is_undirected", lambda _: True)
            graph = glassonion.graph.utils.to_networkx_graph(adj)
    ...
    def test_invalid_adjacency_nonsymmetric(self, adj, monkeypatch):
        ...
        with monkeypatch.context() as m:
            ...
            with pytest.raises(Exception, match="Graphs input as a NumPy array must be real,"):
                glassonion.graph.utils.to_networkx_graph(adj)
    ...
    def test_valid_graph(self, adj):
        ...
        graph = glassonion.graph.utils.to_networkx_graph(nx_graph)
    ...
    def test_invalid_type(self, adj):
        ...
        with pytest.raises(TypeError, match="Graph is not of valid type"):
            glassonion.graph.utils.to_networkx_graph(adj.tolist())
...
class TestIsUndirected:
    ...
    def test_invalid_type(self, adj):
        ...
        with pytest.raises(TypeError, match="Input matrix must be a NumPy array"):
            glassonion.graph.utils.is_undirected(adj.tolist())
...
class TestSubgraphAdjacency:
    ...
    def test_input(self, dim, graph, monkeypatch):
        ...
        with monkeypatch.context() as m:
            m.setattr("glassonion.graph.utils.to_networkx_graph", lambda x: x)
            ...
            assert np.allclose(
                ...
                [glassonion.graph.utils.subgraph_adjacency(graph, s) for s in subgraphs],
            )
    ...
    def test_input_bad_nodes(self, graph, dim, monkeypatch):
        ...
        with monkeypatch.context() as m:
            ...
            with pytest.raises(ValueError, match="Must input a list of subgraph"):
                glassonion.graph.utils.subgraph_adjacency(graph, nodes)
...
class TestIsSubgraph:
    ...
    def test_invalid_type(self, graph):
        ...
        with pytest.raises(TypeError, match="subgraph and graph.nodes must be iterable"):
            glassonion.graph.utils.is_subgraph(None, graph)
    ...
    def test_valid_subgraphs(self, graph, dim):
        ...
        assert all([glassonion.graph.utils.is_subgraph(list(s), graph) for s in subgraphs])
    ...
    def test_invalid_subgraphs(self, graph, dim):
        ...
        assert not all([glassonion.graph.utils.is_subgraph(list(s), graph) for s in subgraphs])
