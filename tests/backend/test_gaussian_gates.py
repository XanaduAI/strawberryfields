...
from strawberryfields.backends.tfbackend.ops import choi_trick, n_mode_gate, single_mode_gate, two_mode_gate, gaussian_gate, gaussian_gate_matrix
...
class TestUnitaryFunctionRelated:
    ...
    def test_choi_trick(self, num_mode, tol):
        ...
        _y = np.block([ np.conj(alpha).T @ W @ np.diag(np.tanh(r)) @ W.T + alpha.T, -np.conj(alpha).T @ W @ np.diag(1/np.cosh(r)) @ V])
...
class TestFockRepresentation:
    ...
    def test_gaussian_gate_matrix_with_fock_tensor(self, num_mode, cutoff, tol):
        ...
        ref_state = np.transpose(fock_tensor(S, (d[:num_mode]+1j*d[num_mode:])/2, cutoff), (0,2,1,3) if num_mode==2 else (0,3,1,4,2,5))
        ...
        ref_state = np.transpose(fock_tensor(S[0], (d[0, :num_mode]+1j*d[0, num_mode:])/2, cutoff), (0,2,1,3) if num_mode==2 else (0,3,1,4,2,5))
        assert np.allclose(Ggate_matrix[0], ref_state, atol=tol, rtol=0.0)
        ref_state = np.transpose(fock_tensor(S[2], (d[2, :num_mode]+1j*d[2, num_mode:])/2, cutoff), (0,2,1,3) if num_mode==2 else (0,3,1,4,2,5))
