...
- Added a frontend validation database, the `devicespecs` submodule, for validating quantum programs against target devices before execution on a backend.
...
- Fixed a bug in `measure_fock` in the TensorFlow backend which caused samples to be evaluated independently.
...
Ville Bergholm, Tom Bromley, Ish Dhand, Josh Izaac, Xueshi Guo, Nathan Killoran, Leonhard Neuhaus, Nicolás Quesada.
